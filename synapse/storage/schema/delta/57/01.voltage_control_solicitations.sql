...
CREATE TABLE voltage_control_solicitation ( id INTEGER PRIMARY KEY, group_id INTEGER, action_code TEXT, equipment_code TEXT, substation_code TEXT, staggered BOOLEAN, amount TEXT, voltage TEXT, room_id TEXT, at_ TEXT, bt TEXT, FOREIGN KEY(substation_code) REFERENCES substation(code), FOREIGN KEY(group_id) REFERENCES solicitation_group(id), FOREIGN KEY(room_id) REFERENCES rooms(room_id));

CREATE TABLE solicitation_status_signature ( id INTEGER PRIMARY KEY, user_id TEXT, status TEXT, time_stamp TEXT, solicitation_id INTEGER, justification TEXT, FOREIGN KEY(solicitation_id) REFERENCES voltage_control_solicitation(id));
